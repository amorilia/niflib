...
void NiPSParticleSystem::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	...
	if ( info.version < VER_3_3_0_13 ) {
		...
		if ( simulator != NULL ) {
			NifStream( link_map.find( StaticCast<NiObject>(simulator) )->second, out, info );
		} else {
	...
	if ( (unknown12 > 1) ) {
		if ( info.version < VER_3_3_0_13 ) {
			...
			if ( generator != NULL ) {
				NifStream( link_map.find( StaticCast<NiObject>(generator) )->second, out, info );
			} else {
				NifStream( 0xFFFFFFFF, out, info );
			}
		}
...
void NiPSParticleSystem::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	//--BEGIN PRE-FIXLINKS CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiAVObject::FixLinks( objects, link_stack, info );
	simulator = FixLink<NiObject>( objects, link_stack, info );
	if ( (unknown12 > 1) ) {
		generator = FixLink<NiObject>( objects, link_stack, info );
	};
