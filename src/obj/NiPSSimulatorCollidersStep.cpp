...
void NiPSSimulatorCollidersStep::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	//--BEGIN PRE-WRITE CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiPSSimulatorStep::Write( out, link_map, info );
	...
	for (unsigned int i1 = 0; i1 < colliders.size(); i1++) {
		if ( info.version < VER_3_3_0_13 ) {
			...
			if ( colliders[i1] != NULL ) {
				NifStream( link_map.find( StaticCast<NiObject>(colliders[i1]) )->second, out, info );
			} else {
...
void NiPSSimulatorCollidersStep::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	//--BEGIN PRE-FIXLINKS CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiPSSimulatorStep::FixLinks( objects, link_stack, info );
	for (unsigned int i1 = 0; i1 < colliders.size(); i1++) {
		colliders[i1] = FixLink<NiObject>( objects, link_stack, info );
	};
