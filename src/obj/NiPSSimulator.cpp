...
void NiPSSimulator::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	//--BEGIN PRE-WRITE CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiMeshModifier::Write( out, link_map, info );
	...
	for (unsigned int i1 = 0; i1 < simulationSteps.size(); i1++) {
		if ( info.version < VER_3_3_0_13 ) {
			...
			if ( simulationSteps[i1] != NULL ) {
				NifStream( link_map.find( StaticCast<NiObject>(simulationSteps[i1]) )->second, out, info );
			} else {
...
void NiPSSimulator::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	//--BEGIN PRE-FIXLINKS CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiMeshModifier::FixLinks( objects, link_stack, info );
	for (unsigned int i1 = 0; i1 < simulationSteps.size(); i1++) {
		simulationSteps[i1] = FixLink<NiPSSimulatorStep>( objects, link_stack, info );
	};
