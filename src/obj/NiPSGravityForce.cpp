...
void NiPSGravityForce::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	...
	if ( info.version < VER_3_3_0_13 ) {
		...
		if ( unknown36 != NULL ) {
			NifStream( link_map.find( StaticCast<NiObject>(unknown36) )->second, out, info );
		} else {
...
void NiPSGravityForce::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	//--BEGIN PRE-FIXLINKS CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiObject::FixLinks( objects, link_stack, info );
	unknown36 = FixLink<NiObject>( objects, link_stack, info );
