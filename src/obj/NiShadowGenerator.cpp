...
void NiShadowGenerator::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	...
	for (unsigned int i1 = 0; i1 < unknownLinks1.size(); i1++) {
		if ( info.version < VER_3_3_0_13 ) {
			...
			if ( unknownLinks1[i1] != NULL ) {
				NifStream( link_map.find( StaticCast<NiObject>(unknownLinks1[i1]) )->second, out, info );
			} else {
...
void NiShadowGenerator::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	...
	for (unsigned int i1 = 0; i1 < unknownLinks1.size(); i1++) {
		unknownLinks1[i1] = FixLink<NiNode>( objects, link_stack, info );
	};
	target = FixLink<NiLight>( objects, link_stack, info );
