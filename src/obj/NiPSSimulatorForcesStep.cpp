...
void NiPSSimulatorForcesStep::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	...
	for (unsigned int i1 = 0; i1 < forces.size(); i1++) {
		if ( info.version < VER_3_3_0_13 ) {
			...
			if ( forces[i1] != NULL ) {
				NifStream( link_map.find( StaticCast<NiObject>(forces[i1]) )->second, out, info );
			} else {
...
void NiPSSimulatorForcesStep::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	...
	for (unsigned int i1 = 0; i1 < forces.size(); i1++) {
		forces[i1] = FixLink<NiObject>( objects, link_stack, info );
	};
