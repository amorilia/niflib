...
void NiPSPlanarCollider::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, const NifInfo & info ) const {
	...
	if ( info.version < VER_3_3_0_13 ) {
		...
		if ( unknownLink6 != NULL ) {
			NifStream( link_map.find( StaticCast<NiObject>(unknownLink6) )->second, out, info );
		} else {
...
void NiPSPlanarCollider::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, const NifInfo & info ) {
	//--BEGIN PRE-FIXLINKS CUSTOM CODE--//

	//--END CUSTOM CODE--//

	NiObject::FixLinks( objects, link_stack, info );
	unknownLink6 = FixLink<NiNode>( objects, link_stack, info );
